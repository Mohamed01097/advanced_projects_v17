...
class TestDynamicPdfReportFormulaEngine(TransactionCase):
    ...
    def test_unknown_field_is_rejected(self):
        report = self._create_report()
        with self.assertRaisesRegex(ValidationError, "references unknown field"):
            self.formula_model.create({
                "report_id": report.id,
                "name": "Unknown Field",
                "formula_type": "arithmetic",
                "formula_expression": "does_not_exist + 1",
            })

    def test_literal_division_by_zero_is_rejected(self):
        report = self._create_report()
        with self.assertRaisesRegex(ValidationError, "divides by zero"):
            self.formula_model.create({
                "report_id": report.id,
                "name": "Literal Division By Zero",
                "formula_type": "arithmetic",
                "formula_expression": "color / 0",
            })

    def test_runtime_division_by_zero_has_readable_error(self):
        report = self._create_report()
        partner = self.env["res.partner"].create({"name": "Zero Partner", "color": 0})
        formula = self.formula_model.create({
            "report_id": report.id,
            "name": "Runtime Division By Zero",
            "formula_type": "arithmetic",
            "formula_expression": "1 / color",
        })
        with self.assertRaisesRegex(UserError, "division by zero"):
            self._compute(formula, partner)

    def test_invalid_syntax_is_rejected(self):
        report = self._create_report()
        with self.assertRaisesRegex(ValidationError, "invalid syntax"):
            self.formula_model.create({
                "report_id": report.id,
                "name": "Invalid Syntax",
                "formula_type": "arithmetic",
                "formula_expression": "color +",
            })

    def test_import_attribute_and_builtin_access_are_rejected(self):
        report = self._create_report()
