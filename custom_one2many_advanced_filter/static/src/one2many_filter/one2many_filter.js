...
extendListRendererProps(ListRenderer);

function extendListRendererProps(Renderer) {
    if (!Renderer) {
        return;
    }
    const props = Renderer.props;
    if (Array.isArray(props)) {
        const optionalProp = `${O2MAF_RENDERER_PROP}?`;
        if (!props.includes(optionalProp) && !props.includes(O2MAF_RENDERER_PROP)) {
            Renderer.props = [...props, optionalProp];
        }
        return;
    }
    if (props && typeof props === "object") {
        const currentProp = props[O2MAF_RENDERER_PROP];
        if (!currentProp) {
            Renderer.props = {
                ...props,
                [O2MAF_RENDERER_PROP]: { type: Object, optional: true },
            };
        } else if (typeof currentProp === "object" && !currentProp.optional) {
            Renderer.props = {
                ...props,
                [O2MAF_RENDERER_PROP]: { ...currentProp, optional: true },
            };
        }
    }
}

function rendererAcceptsFilterStateProp(Renderer) {
    const props = Renderer?.props;
    if (Array.isArray(props)) {
        return props.includes(`${O2MAF_RENDERER_PROP}?`) || props.includes(O2MAF_RENDERER_PROP);
    }
    return Boolean(props && typeof props === "object" && props[O2MAF_RENDERER_PROP]);
}
...
function hasRowKey(rowKeys, rowKey) {
    return Boolean(rowKeys && typeof rowKeys.has === "function" && rowKeys.has(rowKey));
}
...
patch(X2ManyField.prototype, {
    ...
    get rendererProps() {
        ...
        if (this.o2mafCanDisplay) {
            const Renderer = this.o2mafListRenderer;
            extendListRendererProps(Renderer);
            if (rendererAcceptsFilterStateProp(Renderer)) {
    ...
    get o2mafListRenderer() {
        return this.constructor.components?.ListRenderer || X2ManyField.components.ListRenderer;
    },
    ...
    o2mafIsColumnInvisible(column) {
        ...
        if (modifier === true || modifier === "True" || modifier === "1") {
            return true;
        }
        if (!modifier || modifier === false || modifier === "False" || modifier === "0") {
            return false;
        }
        try {
            return evaluateBooleanExpr(String(modifier), this.list?.evalContext || {});
        } catch {
            return false;
        }
    },
...
patch(ListRenderer.prototype, {
    getRowClass(record) {
        ...
        const isMatched = hasRowKey(filter.matchedRowKeys, rowKey);
        const isVisible = hasRowKey(filter.visibleRowKeys, rowKey);
        debugFilter("row", {
            rowKey,
    ...
    get getEmptyRowIds() {
        ...
        const visibleCount = Number.isFinite(filter.visibleCount) ? filter.visibleCount : 0;
        let nbEmptyRow = Math.max(0, 4 - visibleCount);
